...
test("labelName", () => {
    ...
    expect(
        createParse((parser) => parser.labelName())("`TaxOfRiskApp`/`赌博应用`+`TaxOfRiskApp`/`赌博应用`")
    ).toMatchObject({
        type: "LabelName"
    });
});
...
test("labelExpression", () => {
    ...
    expect(
        createParse((parser) => parser.labelExpression())("`TaxOfRiskApp`.`Version`|`TaxOfRiskApp`/`赌博应用`")
    ).toMatchObject({
        type: "LabelExpression"
    });
});
