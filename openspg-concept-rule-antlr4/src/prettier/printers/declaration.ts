    ...
    implements Record<`print${ast.DeclarationNodeType}`, PrintFunc<any>>
{
    ...
    printRuleWrapperDeclaration: PrintFunc<ast.RuleWrapperDeclaration> = ({ path, print }) => [
        ...
        this.block([this.builders.join([this.builders.hardline, this.builders.hardline], path.map(print, "rules"))])
    ];
    ...
    printRuleWrapperRuleDeclaration: PrintFunc<ast.RuleWrapperRuleDeclaration> = ({ node, path, print }) => [
        ...
        this.block(
            this.builders.join([this.builders.hardline, this.builders.hardline], path.map(print, "conceptRules")),
            {
    ...
    printConceptRuleDeclaration: PrintFunc<ast.ConceptRuleDeclaration> = ({ node, path, print }) => [
        ...
        this.block(this.builders.join([this.builders.hardline, this.builders.hardline], path.map(print, "children")), {
            openTag: "{",
            closeTag: "}",
    ...
    printTheActionDeclaration: PrintFunc<ast.TheActionDeclaration> = ({ node, path, print }) => [
        [path.call(print, "head"), this.space],
        this.block(this.builders.join([this.builders.hardline], path.map(print, "expressions")), {
            openTag: "{",
            closeTag: "}",
            empty: node.expressions.length === 0
        })
    ];
    ...
    printTheGraphStructureDeclaration: PrintFunc<ast.TheGraphStructureDeclaration> = ({ node, path, print }) => [
        ...
        this.block(
            ...
            ],
            {
                openTag: "{",
                closeTag: "}",
                empty: node.graphStructures.length === 0 && node.pathPatterns.length === 0
            }
        )
    ];
