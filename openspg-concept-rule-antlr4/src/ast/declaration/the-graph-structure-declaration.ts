...
export class TheGraphStructureDeclaration extends BaseNode {
    ...
    constructor(ctx: TheGraphStructureDeclarationContext, visitor: ConceptRuleParserVisitor<any>) {
        ...
        this.graphStructures = (ctx.theGraphStructureBody()?.graphStructureList()?.graphStructure() || []).map(x => x.accept(visitor))
        this.pathPatterns = (ctx.theGraphStructureBody()?.pathPatternList()?.pathPattern() || []).map(x => x.accept(visitor))
    }
