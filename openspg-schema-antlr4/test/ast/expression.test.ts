...
test('inherited structure declaration', () => {
    expect(createParse((parser) => parser.basicStructureDeclaration())(`Person(人物) -> EntityType, SomeTime:`)).toMatchObject({
        type: 'BasicStructureDeclaration',
        name: {
            type: 'StructureNameExpression', variable: {
                realName: 'Person'
        ...
        structureType: {
            type: 'InheritedStructureTypeExpression', variables: ['EntityType', {
                type: 'StructureName', realName: 'SomeTime'
            }]
        }
    });
});
...
test('basic property declaration', () => {
    expect(createParse((parser) => parser.basicPropertyDeclaration())(`desc: text`)).toMatchObject({
        ...
        name: {
            type: 'PropertyNameExpression', variable: 'desc'
        },
        value: {
