...
export class PrinterExpression extends BasePrinter implements Record<`print${ast.ExpressionNodeType}`, PrintFunc<any>> {
    ...
    printInheritedStructureTypeExpression: PrintFunc<ast.InheritedStructureTypeExpression> = ({ path, print }) => [
        "->",
        this.space,
        this.builders.join([this.comma, this.space], path.map(print, "variables")),
        this.colon
    ];
    ...
    printStructureName: PrintFunc<ast.StructureName> = ({ path, print }) => [
        this.builders.join("#", [...path.map(print, "semanticNames"), path.call(print, "realName")])
    ];
    ...
    printBlockPropertyValue: PrintFunc<ast.BlockPropertyValue> = ({ node, path, print }) => [
        this.block(path.call(print, "content"), {
            openTag: "[[",
            closeTag: "]]",
            empty: node.content.text.trim().length === 0
        })
    ];
}
