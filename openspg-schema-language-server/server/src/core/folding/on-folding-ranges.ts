...
export const onFoldingRanges: OnFoldingRanges =
    ...
    async ({ textDocument }) => {
        ...
        const trimRange = (range: [number, number]) => {
            const plainContent = document.getText(
                Range.create(document.positionAt(range[0]), document.positionAt(range[1]))
            );
        ...
        syntax.visit(document.ast, {
            EntityDeclaration: ({ node }) => {
                if (node.children && node.children.length > 0) {
                    ...
                    foldingRanges.push(
                        FoldingRange.create(document.positionAt(range[0]).line, document.positionAt(range[1]).line)
                    );
                }
            },
            ...
            BlockPropertyValue: ({ node }) => {
                ...
                if (start.line < end.line) {
                    foldingRanges.push(FoldingRange.create(start.line, end.line));
                }
            }
        });
