...
class Printer implements Record<string, PrintFunc<any>> {
    ...
    printKnowledgeStructureType: PrintFunc<syntax.KnowledgeStructureType> = (node) => this.bold(node.text);

    printBasicStructureType: PrintFunc<syntax.BasicStructureType> = (node) => this.bold(node.text);
