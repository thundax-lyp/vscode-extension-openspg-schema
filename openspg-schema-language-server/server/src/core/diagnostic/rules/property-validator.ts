            ...
            nodes.forEach(x => {
                ...
                if (existName) {
                    diagnostics.push(Diagnostic.create(
                        document.getNodeRange(x.declaration.name), `Cannot redeclare block-scoped property \"${displayName}\"`, DiagnosticSeverity.Error
                    ));
                    if (existName.repeat === 1) {
                        diagnostics.push(Diagnostic.create(
                            document.getNodeRange(existName.node.declaration.name), `Cannot redeclare block-scoped property \"${displayName}\"`, DiagnosticSeverity.Error
                        ));
                    }
