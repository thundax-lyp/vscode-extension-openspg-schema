...
export const onDocumentSymbol: OnDocumentSymbol =
    (ctx) =>
        async ({ textDocument }) => {
            ...
            const handleRuleWrapperDeclaration = async (ast: syntax.RuleWrapperDeclaration): Promise<DocumentSymbol> => {
                ...
                return {
                    ...
                    children: await Promise.all(
                        ast.rules.flatMap((x) => x.conceptRules).map((x) => handleConceptRuleDeclaration(x))
                    )
                };
            };
            ...
            const handleTheGraphStructureDeclaration = async (
                ast: syntax.TheGraphStructureDeclaration
