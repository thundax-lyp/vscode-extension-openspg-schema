...
export class ConceptRuleDeclarationValidator implements Validator {
    ...
    public validate(document: ConceptRuleTextDocument) {
        ...
        const addDiagnostic = (node: syntax.SyntaxNode, message: string, severity: DiagnosticSeverity = DiagnosticSeverity.Error) => {
                ...
                if (item.type === 'TheGraphStructureDeclaration') {
                    ...
                    if (theRuleExists) {
                        ...
                        if (idx > 0 && declaration.children[idx - 1].type !== 'TheGraphStructureDeclaration') {
                            addDiagnostic(item, `"Rule/Constraint" should be the next of "Structure/GraphStructure" in the scope`);
                        }
                    }
        ...
        const globalConceptRules: ConceptRuleDeclaration[] = document.ast!.nodes.filter(x => x.type === 'ConceptRuleDeclaration');
